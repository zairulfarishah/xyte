...
//
// site_assignments.assignment_date holds the day a row applies to. A row with
...
export function hasDailyCrew(assignments = []) {
  return assignments.some(a => a.assignment_date)
}
...
export function assignmentsForDate(assignments = [], date) {
  ...
  const undated = assignments.filter(a => !a.assignment_date)
  if (!target) return undated.length > 0 ? undated : assignments

  const dated = assignments.filter(a => normalizeDate(a.assignment_date) === target)
...
export function memberDatesOnSite(site, memberId) {
  ...
  if (mine.some(a => !a.assignment_date)) return getSiteDates(site)

  const siteDates = getSiteDates(site)
  const mineDates = new Set(mine.map(a => normalizeDate(a.assignment_date)))
...
export function assignmentDays(site) {
  ...
  return assignments.flatMap(a => {
    if (a.assignment_date) return [{ assignment: a, date: normalizeDate(a.assignment_date) }]
